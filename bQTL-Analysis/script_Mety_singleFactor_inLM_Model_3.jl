...
# sink55 = DataFrame(MOA_window = String[], SNP = String[], log10ProbMethy = Union{Missing, Float64}[], numberOFsnps = Int64[], ExpVariance = Float64[])
# sink44 = DataFrame(MOA_window = String[], SNP = String[], log10ProbMethy = Union{Missing, Float64}[], numberOFsnps = Int64[], ExpVariance = Float64[])
# sink33 = DataFrame(MOA_window = String[], SNP = String[], log10ProbMethy = Union{Missing, Float64}[], numberOFsnps = Int64[], ExpVariance = Float64[])
...
# Results = Dict(:CG => sink33, :CHG => sink44, :CHH => sink55, :METHonly => sink77)


         Results = DataFrame(MOA_window = String[], log10ProbCHGsingle = Union{Missing, Float64}[], 
                                           log10ProbCGsingle = Union{Missing, Float64}[],
                                           log10ProbCHHsingle = Union{Missing, Float64}[],
                                           log10ProbCGcombi = Union{Missing, Float64}[], 
                                           log10ProbCHGcombi = Union{Missing, Float64}[],
                                           log10ProbCHHcombi = Union{Missing, Float64}[],
                                           ExpVarianceCHG = Float64[],
                                           ExpVarianceCG = Float64[],
                                           ExpVarianceCHH = Float64[],
                                           ExpVariacneMix = Float64[]
                                           )
...
@showprogress for itt in 1:MOA
                    ...
                    #try
                            ...
                            =#
                                PvaluesMM = DataFrame(MethCHG = Union{Missing, Float64}[], 
                                ...
                                eVarMM = DataFrame( CHG = Float64[], CG = Float64[], CHH = Float64[], METH = Float64[])
                        ...
                        try  # Mixed model 
                            ...
                            for i in names(canM)[7:length(names(canM))] 
                                ...
                                #### the R veraion - 2.585 seconds
                                                                        ...
                                                                        @rput canM
                                                                                ...
                                                                                a2 = summary(lm(MOA ~ CHG , data=canM))$coefficients
                                                                                a3 = summary(lm(MOA ~ CG , data=canM))$coefficients
                                                                                a4 = summary(lm(MOA ~ CHH , data=canM))$coefficients
                                                                                ...
                                                                                pvals = -log10(pvals)
                                                                                        ...
                                                                                        a2 = anova(lm(MOA ~ CHG, data=canM))
                                                                                        a3 = anova(lm(MOA ~ CG , data=canM))
                                                                                        a4 = anova(lm(MOA ~ CHH, data=canM))
                                                                                        #a5 = anova(lm(MOA ~ canM[,i] , data=canM))
                                                                                        a6 = anova(lm(MOA ~ CHG + CG + CHH , data=canM, na.action="na.exclude"))
                            ...
                            # push!(Results[:CG], [moa.ID[itt], SNP, pvalmax, dpmet, ncol(canM)-6, ev])
                            ...
                            # push!(Results[:CHG], [moa.ID[itt], SNP, pvalmax, dpmet, ncol(canM)-6, ev])
                            ...
                            # push!(Results[:CHH], [moa.ID[itt], SNP, pvalmax, dpmet, ncol(canM)-6, ev])
                            ...
                            # # push!(Results[:SNPonly], [moa.ID[itt], SNP, pvalmax, ncol(canM)-6, ev])
                            ...
                            ## all info in a single table 
                                ...
                                # Results = DataFrame(MOA_window = String[], log10ProbCGsingle = Union{Missing, Float64}[], 
                                #            log10ProbCHGsingle = Union{Missing, Float64}[],
                                #            log10ProbCHHsingle = Union{Missing, Float64}[],
                                #            log10ProbCGcombi = Union{Missing, Float64}[], 
                                #            log10ProbCHGcombi = Union{Missing, Float64}[],
                                #            log10ProbCHHcombi = Union{Missing, Float64}[],
                                ...
                                #            ExpVariacneMix = Float64[]
                                #            )
                                push!(Results, hcat(moa.ID[itt],Matrix(PvaluesMM), Matrix(eVarMM)))
...
disk = "/netscratch/dep_psl/grp_frommer/Thomas/Results/HybMoa_0819_WWvsDS/bQTLs_25/NG_revision/Results/NPNRtoValue/LM/$con/"
#disk = "$path"


                                LOC = string(disk, con,"-" ,"MOA_", me, ".csv")
                                CSV.write(LOC, Results)
