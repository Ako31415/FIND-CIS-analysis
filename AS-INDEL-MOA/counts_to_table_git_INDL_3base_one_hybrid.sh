#!/bin/bash
#run by ./counts_to_table.sh genotype dir cond1 cond2 Peaks_cond1 Peaks_cond2 bedgraph_condition1 bedgraph_condition2 
EXPECTED_ARGS=8
E_BADARGS=9
if [ $# -ne $EXPECTED_ARGS ]
then
  echo "Usage: ./counts_to_table.sh genotype dir cond1 cond2 Peaks_cond1 Peaks_cond2 bedgraph_condition1 bedgraph_condition2  "
  exit $E_BADARGS
fi
...
for tr in ${C1} ${C2}; do gawk -v OFS='\t' '{print $0,"1/1",$1":"$2+3}' ${g}.B73.${tr}.ID.count.sort.csv > ${g}.B73.${tr}.ID.count.sort.geno.csv;done
...
for tr in ${C1} ${C2}; do gawk -v OFS='\t' '{if(NR==FNR) {a[$4]=$4;Value[$4]=$5;Peak[$4]=$9;geno[$4]=$10;ID[$4]=$11; next} if($4 in a){split($4,var,"_");split(var[1],AL,":");if(length(AL[1])>1){print $1,$2,AL[1],AL[2],ID[$4],geno[$4],$5,Value[$4],$6,Peak[$4]}else{print $1,$2+3,AL[1],AL[2],ID[$4],geno[$4],$5,Value[$4],$6,Peak[$4]}}}' ${g}.B73.${tr}.ID.count.sort.geno.csv B73.${g}.${tr}.ID.count.sort.csv |  gawk -v OFS='\t' '{if($7+$8>0){if($9~"peak" || $10~"peak"){print $0,($7/($7+$8))}else{print $0,"n.p."($7/($7+$8))}}else{print $0,"n.p.n.r."}}'  > B73.${g}.${tr}.PF.GT.csv; done
